...
// GPT-4o Mini transcribe: más económico ($0.003/min), calidad intermedia.
const GPT4O_MINI_TRANSCRIBE_MODEL = "gpt-4o-mini-transcribe";
...
async function loadInitialPrompt() {
  try {
    ...
    // GPT-4o Mini soporta prompts más largos que Whisper - usamos hasta 4000 chars
...
export async function transcribeAudioGpt4oMini(formData) {
  ...
  try {
    ...
    const estimatedCost = (audioDurationSeconds / 60) * 0.003;

    console.log(`🧠 GPT-4o Mini Transcribe Request iniciada (${serverUsage.requests + 1}/${MAX_DAILY_REQUESTS} del día):`);
    console.log(`   - Modelo: ${GPT4O_MINI_TRANSCRIBE_MODEL}`);
    ...
    // Llamar a la API de transcripción con GPT-4o Mini
    const transcription = await openai.audio.transcriptions.create({
      file: file,
      model: GPT4O_MINI_TRANSCRIBE_MODEL,
      language: "es",
    ...
    console.log(`✅ GPT-4o Mini Transcribe Response recibida:`);
    ...
    console.error("❌ Error en transcripción GPT-4o Mini:", error);
    ...
    return {
      error:
        error.message || "Error al transcribir con GPT-4o Mini. Intentá de nuevo.",
    };
  }
}
